...
class User:
    def __init__(self, first_name:str, last_name:str, email:str, password:str, role:str, profilePic=None, last_edit:str=None, status:bool=None, id:int=None):
        self.first_name = first_name
        self.last_name = last_name
        self.email = email
        self.password = bcrypt.hashpw(password.encode(), bcrypt.gensalt()).decode() if password else None
        self.role = role
...
def CreateUser(cursor, user: User):
    if cursor:
        ...
        cursor.execute(sql_fetch_query, (user.first_name, user.last_name, user.email, user.password, user.role, user.profilePic.read() if user.profilePic else None))
        cursor.fetchone()
...
def UpdateUser(cursor, user: User):
    if cursor and user.id:
        ...
        cursor.execute(sql_update_query, (user.first_name, user.last_name, user.email, user.role, user.profilePic, user.status, user.id))
